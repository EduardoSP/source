...
function Tracker(connection, options) {
  ...
  this.connection.on('data', function(buffer) {
    console.log("MENSAJE PURO");
    console.log(buffer.toString('hex').toString(16));
    console.log(buffer.toString('hex'));
    var datohex = buffer.toString('hex').substring(26,28);
    console.log(datohex);
    ...
    if (message.command === Message.commands.REQUEST_SEND_PHOTO) {
      
      console.log("REQUEST_SEND_PHOTO");
	
      console.log(message.data);
      console.log(message.data.toString(16));
      //console.log(new Buffer(message.data, 'hex').toString('ascii'));
      //console.log(new Buffer(message.data, 'hex').toString(16));
      //console.log(new Buffer(message.data, 'hex'));
      console.log("------------------------")
       
      tracker.send(new Message({
        command : Message.commands.ACCEPT_SEND_PHOTO,
        data    : datohex,//message.data.toString(16),
        mode    : Message.MODE_RAW_DATA
      }));
      tracker.emit('login');
 ...
 * @param {function} callback
 */
Tracker.prototype.getPhoto = function(callback) {
  this.request(new Message({command: Message.commands.GET_PHOTO}), function(error, response) {
    if (error) {
      callback(error);
      return;
    }
    callback(null, Message.parseCommandResult(Message.commands.GET_PHOTO, new Buffer(response.data)));
  }); 
};
